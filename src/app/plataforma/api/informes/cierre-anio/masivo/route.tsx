...
import { CertificadoCierreAnioPage } from "@/lib/pdf/CertificadoCierreAnioDocument";
...
/** Impresión masiva del Informe de Cierre de Año de un curso completo -- ver comentario equivalente en informes/semestral/masivo/route.tsx. */
export async function POST(request: Request) {
  ...
  if (!isEnsenanzaBasica(course.level)) {
    return NextResponse.json({ error: "El Informe de Cierre de Año aplica solo a cursos de Enseñanza Básica (1° a 8° Básico)" }, { status: 400 });
  }
  ...
  const nextCourseFormalName = nextFormalCourseName(course.level);
  const issuedAt = new Date().toISOString();

  const buffer = await renderToBuffer(
    <Document title={`Informes de Cierre de Año - ${course.level} ${course.letter}`}>
      {available.map((report) => {
        const promoted = report.generalAverage !== null && report.generalAverage >= DEFAULT_GRADING_CONFIG.approvalMinimum;
        return (
          <CertificadoCierreAnioPage
            key={report.studentId}
            folio="—"
            studentName={report.studentName}
            studentRun={report.studentRun}
            courseFormalName={courseFormalName}
            nextCourseFormalName={nextCourseFormalName}
            year={year.year}
            rows={report.rows}
  ...
  await supabase.rpc("log_audit", {
    ...
    p_details: { cert_type: "cierre_anio", academic_year_id, student_count: available.length, pending_count: reports.length - available.length },
  });

  return new NextResponse(new Uint8Array(buffer), {
    headers: {
      "Content-Type": "application/pdf",
      "Content-Disposition": `inline; filename="informes-cierre-anio-${course.level}-${course.letter}.pdf"`,
    },
  });
}
