...
export default async function EquipoPieAdminPage() {
  const [items, people, session] = await Promise.all([
    listSectionMemberships("pie"),
    listAllStaffMembers(),
    getSessionContext(),
  ]);
  ...
  return (
    <div>
      <h1 className="text-2xl font-semibold text-slate-900">Equipo PIE</h1>
      <p className="mt-1 text-sm text-slate-500">Administra quién aparece en la página pública de Equipo PIE.</p>
      ...
      <div className={`mt-6 grid gap-6 ${allowedToWrite ? "lg:grid-cols-[1fr_400px]" : ""}`}>
        <Card>
          <CardBody>
            {items.length > 0 ? (
                  ...
                  <li key={m.id} className="flex items-center justify-between gap-3 py-3">
                    ...
                    <div className="flex shrink-0 items-center gap-1">
                      ...
                      {allowedToWrite && (
                        <>
                          <Link
                            href={`/plataforma/equipo-institucional/pie/${m.id}`}
              ...
              <EmptyState icon={HeartHandshake} title="Sin integrantes" description="Agrega la primera persona al Equipo PIE." />
            )}
          </CardBody>
        </Card>
        ...
        {allowedToWrite && (
          <Card>
            <CardBody>
              <h2 className="font-semibold text-slate-900">Agregar al Equipo PIE</h2>
              <div className="mt-4">
                <PersonRoleForm
                  target={{ kind: "membership", section: "pie" }}
                  existingPeople={people}
                  redirectTo="/plataforma/equipo-institucional/pie"
                  auditModule="equipo-institucional"
                />
              </div>
            </CardBody>
