...
import { FileText, Download, FolderOpen, Megaphone, Eye, ScrollText } from "lucide-react";
import { PageHeader } from "@/components/public/PageHeader";
import { EmptyState } from "@/components/ui/EmptyState";
import { Badge } from "@/components/ui/Badge";
import { formatDate } from "@/lib/utils";
import { formatBulletinDate } from "@/lib/bulletin-content";
import { getPublicDocuments, getPublishedBulletins, getPublicCircularesInformativas } from "@/services/public-content";

export const metadata: Metadata = { title: "Documentos Institucionales" };

export default async function DocumentosPage() {
  const [documents, bulletins, circulares] = await Promise.all([
    getPublicDocuments(),
    getPublishedBulletins(),
    getPublicCircularesInformativas(),
  ]);
    ...
    <>
      ...
      <section className="mx-auto max-w-4xl px-4 pb-14 sm:px-6 lg:px-8">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-heading text-xl font-medium tracking-tight text-slate-900">Circulares Informativas</h2>
        </div>

        {circulares.length > 0 ? (
          <ul className="mt-5 divide-y divide-slate-200 overflow-hidden rounded-xl border border-slate-200 bg-white">
            {circulares.map((c) => (
              <li key={c.id} className="flex flex-wrap items-center justify-between gap-4 p-5">
                <div className="flex items-start gap-4">
                  <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-brand-50 text-brand-700">
                  ...
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {c.document_number && <Badge tone="brand">N.º {c.document_number}</Badge>}
                    </div>
                    <p className="mt-1 text-sm text-slate-500">{formatDate(c.circular_date)}</p>
                ...
                <div className="flex shrink-0 flex-wrap items-center gap-2">
                  ...
                  <a
                    href={`/api/circulares-informativas/${c.id}/pdf`}
                    className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 px-3.5 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50"
                  >
          ...
          <div className="mt-5">
            <EmptyState
              icon={ScrollText}
              title="No hay circulares informativas publicadas actualmente."
              description="Las circulares emitidas por el establecimiento se publicarán aquí."
            />
          </div>
        )}
      </section>
